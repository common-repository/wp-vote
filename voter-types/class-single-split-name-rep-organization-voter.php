 ...
 * @package WP_Vote
 */
class Single_Split_Name_Rep_Organization_Voter extends Abstract_Voter_Object {
	...
	protected static $fields = array(
		...
		'rep_first_name' => array( 'required' => true ),
		'rep_last_name'  => array( 'required' => true ),
	...
	public static function init() {
		self::$slug  = 'single-split-name-rep-organization-voter';
		self::$label = __( 'Single Rep Organization with split name', 'wp-vote' );
	...
	public static function render_meta_fields() {
		...
		$cmb->add_field( array(
			'name'       => __( 'Rep: First Name', 'wp-vote' ),
			'desc'       => '',
			...
			if ( $voter_post ) {
				...
				$this->reps  = array(
					array(
						...
						'rep_first_name' => get_post_meta( $this->post->ID, self::get_prefix( 'rep_first_name' ), true ),
						'rep_last_name'  => get_post_meta( $this->post->ID, self::get_prefix( 'rep_last_name' ), true ),
					),
				);
