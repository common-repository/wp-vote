...
/**
 * Class Single_Rep_Organization_Voter
 * @package WP_Vote
 */
class Single_Rep_Organization_Voter extends Abstract_Voter_Object {
...
//		'member_name' => array( 'required' => true ),
		...
		'rep_name' => array( 'required' => true ),
		'rep_email' => array( 'required' => true ),
	);
	...
	public static function init() {
		self::$slug  = 'single-rep-organization-voter';
		self::$label = __( 'Single Rep Organization', 'wp-vote' );
	...
	public static function render_meta_fields() {
		...
		$cmb->add_field( array(
			'name'       => __( 'Rep: Name', 'wp-vote' ),
			'desc'       => '',
			'id'         => self::get_prefix( 'rep_name' ),
			'type'       => 'text',
			'show_names' => true, // Show field names on the left
			'attributes' => array(
				'required' => 'required',
		...
		$cmb->add_field( array(
			'name'       => __( 'Rep: Email', 'wp-vote' ),
			'desc'       => '',
			'id'         => self::get_prefix( 'rep_email' ),
			'type'       => 'text_email',
			'show_names' => true, // Show field names on the left
			'attributes' => array(
				'required' => 'required',
...
//
//		$valid_rep_email = is_email( $voter_meta[ self::get_prefix( 'rep_email' ) ] );
//
//		if ( ! empty( $voter_meta ) ) {
...
//		if ( $valid_fields && $valid_rep_email ) {
...
//			}
//			if ( ! $valid_rep_email ) {
//				Import_Logger::warn( "<a href='$permalink' target='_blank'>{$voter_meta[self::get_prefix('member_id')]}: {$voter_meta[self::get_prefix('member_name')]} - Rep Email Invalid</a>" );
//			}
//
//		}
//
	...
	public function __construct( $args ) {
		...
		if ( ! is_array( $args ) ) {
			...
			if ( $voter_post ) {
				...
				$this->reps  = array(
					array(
						'email' => get_post_meta( $this->post->ID, self::get_prefix( 'rep_email' ), true ),
						'name'  => get_post_meta( $this->post->ID, self::get_prefix( 'rep_name' ), true ),
					),
				);
