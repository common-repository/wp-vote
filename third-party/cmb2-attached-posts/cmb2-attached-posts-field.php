...
class WDS_CMB2_Attached_Posts_Field {
	...
	public function render( $field, $escaped_value, $object_id, $object_type, $field_type ) {
		...
		foreach ( $posts as $post ) {
			...
			// Get Voter Type
			$voter_type = get_post_meta( $post->ID, \WP_Vote\Voter::get_prefix( 'voter_type' ), true );
			// Build our list item
			echo '<li data-id="', $post->ID, '" data-voter-type="' . $voter_type . '" class="' . $zebra . $added . '">', $thumbnail, '<a title="' . __( 'Edit' ) . '" href="', get_edit_post_link( $post ), '">', get_the_title( $post ), '</a><span class="dashicons dashicons-plus add-remove"></span></li>';
	...
	protected function display_attached( $field, $attached ) {
		...
		foreach ( $attached as $post_id ) {
			...
			// Set thumbnail if the options is true
			$thumbnail = $show_thumbnails ? get_the_post_thumbnail( $post_id, array(
				50,
				50
			) ) : '';
